...
void gghc_process_files(gghc_ctx ctx)
{
  ...
  ctx->cmd = ssprintf("%s -E     %s '-D__gghc__' '%s'     > '%s'", ctx->cc_prog, ctx->cc_options, ctx->cpp_in_filename, ctx->cpp_out_filename);
  ...
  ctx->cmd = ssprintf("%s -E -dM %s '-D__gghc__' '%s'     > '%s'", ctx->cc_prog, ctx->cc_options, ctx->cpp_in_filename, ctx->defines_in_filename);
  gghc_system(ctx, ctx->cmd);
  ...
  ctx->cmd = ssprintf("%s        %s '-D__gghc__' '%s' -o '%s'",
          ctx->cc_prog, ctx->cc_options, ctx->constants_c_filename, ctx->constants_x_filename);
